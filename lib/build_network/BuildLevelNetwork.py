...
from lib.models.Building import Room, Door, Level
from lib.utils.geometries import construct_level_cutter
from lib.utils.tools import plot_any

# from dataclasses import field


class BuildLevelNetwork:

    def __init__(self, level: Level):
        # XXX: is it better to build the whole network or do it per level like we're doing now?
        ...
        self.exit_nodes = []
        self.level = level
        self.cutter = construct_level_cutter(level)
    ...
    def add_door_centroids(self):
        for door in enumerate(self.level.doors):
            self.graph.add_node(str(door[0]), pos=door[1].centroid[0:2], type=door[1].type, color="yellow")
            self.graph.add_node(str(door[0]) + "upper", pos=door[1].upper_centroid[0:2], type=door[1].type,
                                color="yellow")
            self.graph.add_node(str(door[0]) + "lower", pos=door[1].lower_centroid[0:2], type=door[1].type,
                                color="yellow")
            if door[1].type == "exit":
    ...
    def get_furthest_node_from_door(self, room: Room, intersecting_doors: list):
        ...
        maxes = [-1.0, "node"]
        for coordinate in enumerate(room.room_coordinates):
            for intersecting_door in intersecting_doors:
                distance = self.calculate_distance(coordinate[1], intersecting_door.centroid)
                if distance > maxes[0]:
                    maxes[0] = distance
                    maxes[1] = str(room.room_number) + str(coordinate[0])
        return maxes[1]

    def add_room_coordinates(self):
        for room in enumerate(self.level.rooms):
            coordinates = room[1].room_coordinates
            ...
            for coordinate in enumerate(coordinates):
                name = str(room[1].room_number) + str(coordinate[0])
                if name == furthest_node:
                    self.start_nodes.append(str(room[1].room_number) + str(coordinate[0]))
                    self.graph.add_node(str(room[1].room_number) + str(coordinate[0]), pos=(coordinate[1][0:2]),
                                        id=room[1].room_number, start=True, color="green")
                else:
                    self.graph.add_node(str(room[1].room_number) + str(coordinate[0]), pos=(coordinate[1][0:2]),
                                        id=room[1].room_number, start=False, color="blue")

    # this is just for labelling, and should be called after all edges have been drawn
    # if called before edges are drawn, the path from the room will go through the centroid (maybe)
    def add_room_centroids(self):
        for room in enumerate(self.level.rooms):
            self.graph.add_node(str(room[0]) + "centroid", pos=room[1].room_centre_point[0:2], id=room[1].room_number, start=False,
                                color="Red", label=room[1].room_number)

    # stops after two doors
    def find_intersecting_doors(self, room: Polygon) -> list[Door]:
        doors = self.level.doors
        intersecting_doors = []
        count = 0
        for door in doors:
            # TODO instead of intersects, use crosses
    ...
    # creates linestring with start, end coords of each edge and then intersects with cutter to see if edge passes
    # through a wall
    def remove_intersecting_edges(self):
        for edge in self.edge_graph.edges():
            line = LineString([self.graph.nodes[edge[0]]["pos"][0:2], self.graph.nodes[edge[1]]["pos"][0:2]])
    ...
    def find_shortest_paths(self, exit_node: str):
        paths = []
        lengths = []
        self.compute_all_edges()
        self.remove_intersecting_edges()
        for node in self.start_nodes:
            shortest_path = nx.shortest_path(self.edge_graph, source=node, target=exit_node, weight='weight')
            shortest_path_length = nx.shortest_path_length(self.edge_graph, source=node, target=exit_node,
                                                           weight='weight')
    ...
    def plot_all_paths(self, normalise: int, exit_node: str):
        colors = nx.get_node_attributes(self.graph, "color").values()
        labels = nx.get_node_attributes(self.graph, "label")
        pos = nx.get_node_attributes(self.graph, 'pos')
        edges, lengths = self.find_shortest_paths(exit_node=exit_node)
        plot_any(self.cutter, plt)

        for edge, length in zip(edges, lengths):
            hue = float(length / normalise)
    ...
    def plot_single_path(self, normalise: int, start_node_index: int = 0, exit_node: str = "0"):
        colors = nx.get_node_attributes(self.graph, "color").values()
        labels = nx.get_node_attributes(self.graph, "label")
        ...
        edges, lengths = self.find_shortest_paths(exit_node=exit_node)
        print(lengths)
        print("start node: ", edges[start_node_index], "length: ", lengths[start_node_index])
        hex_clr = self.get_hex_color(lengths[start_node_index], normalise)
        plot_any(self.cutter, plt)
