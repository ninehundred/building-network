...
class BuildNetwork:
            ...
            if room[1].level == self.level:
                ...
                for coordinate in enumerate(coordinates):
                    ...
                    if name == furthest_node:
                        ...
                        self.graph.add_node(str(room[1].number) + str(coordinate[0]), pos=(coordinate[1]),
                                            id=room[1].number, start=True, color="green")
                    else:
                        self.graph.add_node(str(room[1].number) + str(coordinate[0]), pos=(coordinate[1]),
                                            id=room[1].number, start=False, color="blue")
    ...
    def add_room_centroids(self):
        for room in enumerate(self.building.rooms):
            if room[1].level == self.level:
                self.graph.add_node(str(room[0]) + "centroid", pos=room[1].centroid, id=room[1].number, start=False,
                                    color="Red", label=room[1].number)
    ...
    def find_shortest_paths(self):
        ...
        for node in self.start_nodes:
            ...
            shortest_path_length = nx.shortest_path_length(self.edge_graph, source=node, target=self.exit_nodes[0],
                                                           weight='weight')
    ...
    def plot_all_paths(self, normalise: int):
    ...
    def plot_single_path(self, normalise: int, start_node_index: int = 0):
        colors = nx.get_node_attributes(self.graph, "color").values()
        labels = nx.get_node_attributes(self.graph, "label")
